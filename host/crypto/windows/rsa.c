    ...
    size_t* signature_size)
{
	...
	
	{
        ...
        OE_CHECK(oe_memcpy_s(hash_data_copy, sizeof(hash_data_copy), hash_data, hash_size));
    ...
    oe_rsa_public_key_t* public_key)
{
	...
	/*
	 * Export the key blob to a public key. Note that the private key blob has the
	 * modulus and the exponent already, so we can just use it to import the public key.
	 */
	{
        NTSTATUS status;
    ...
    size_t* buffer_size)
{
    ...
    /* Check for null parameters and invalid sizes. */
    if (!oe_public_key_is_valid(impl) || !buffer_size || *buffer_size > MAXDWORD)
	{
        OE_RAISE(OE_INVALID_PARAMETER);
	}
